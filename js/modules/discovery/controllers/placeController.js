...
import { renderPlaceDetail, renderPlaceDetailSkeleton } from '../render/renderPlaceDetail.js?v=15';
...
export async function initPlaceController(placeSlug, stateSlug) {
  ...
  try {
    ...
    if (canonicalId && canonicalId !== placeSlug && window.location.hash.includes(placeSlug)) {
       const newHash = window.location.hash.replace(`place=${placeSlug}`, `place=${canonicalId}`);
    ...
    // Render full detail
    renderPlaceDetail(place, container);
