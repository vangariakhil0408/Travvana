 ...
 */
export function renderPlaceDetail(place, container) {
  ...
  const imageHTML = imgSrc && imgSrc !== 'null'
    ? `<img class="detail-hero-img" src="${imgSrc}" alt="${place.name}" onerror="${onErrorHandler()}">`
    : `<div class="detail-hero-img" style="background:linear-gradient(135deg,#f5f5f5,#e8e8e8);display:flex;align-items:center;justify-content:center;flex-direction:column;gap:12px;color:#aaa;font-size:1rem;"><svg width='48' height='48' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='1.5'><rect x='3' y='3' width='18' height='18' rx='3'/><circle cx='8.5' cy='8.5' r='1.5'/><path d='M21 15l-5-5L5 21'/></svg><span>Image coming soon</span></div>`;
  ...
  const html = `
    ...
    <!-- Main Detail Card -->
    <div class="detail-card page-transition-enter page-transition-active">
      <h1 class="detail-title">${escapeHTML(place.name)}</h1>
      ...
      <div class="detail-about-card">
        ...
        <p class="detail-text" style="margin-bottom: 0;">${escapeHTML(place.history || 'Information about this place will be updated soon.')}</p>
      </div>
